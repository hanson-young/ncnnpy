...
public:
    ...
    Rect anchor; // x1,y1,x2,y2
	float reg[4]; // offset reg
    Point center; // anchor feat center
	float score; // cls score
    std::vector<Point> pts; // pred pts

    Rect finalbox; // final box res
...
private:
    ...
    void bbox_pred(const CRect2f& anchor, const CRect2f& delta, Rect& box);

    void landmark_pred(const CRect2f anchor, const std::vector<Point>& delta, std::vector<Point>& pts);
