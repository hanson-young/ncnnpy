...
//                    printf("%f %f %f %f\n", box[0], box[1], box[2], box[3]);
                    ...
                    Anchor res;
                    res.anchor = Rect(box[0], box[1], box[2], box[3]);
...
//                    printf("center %d %d\n", j, i);
                    ...
                    if (1) {
                        std::vector<Point> pts_delta(pts_length);
...
void AnchorGenerator::bbox_pred(const CRect2f& anchor, const CRect2f& delta, Rect& box) {
	float w = anchor[2] - anchor[0] + 1;	
	float h = anchor[3] - anchor[1] + 1;
	float x_ctr = anchor[0] + 0.5 * (w - 1);
	float y_ctr = anchor[1] + 0.5 * (h - 1);
    ...
    box = Rect(pred_ctr_x - 0.5 * (pred_w - 1.0),
            pred_ctr_y - 0.5 * (pred_h - 1.0),
            pred_ctr_x + 0.5 * (pred_w - 1.0),
            pred_ctr_y + 0.5 * (pred_h - 1.0));
}

void AnchorGenerator::landmark_pred(const CRect2f anchor, const std::vector<Point>& delta, std::vector<Point>& pts) {
	float w = anchor[2] - anchor[0] + 1;	
	float h = anchor[3] - anchor[1] + 1;
	float x_ctr = anchor[0] + 0.5 * (w - 1);
	float y_ctr = anchor[1] + 0.5 * (h - 1);
